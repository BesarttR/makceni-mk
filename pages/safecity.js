...
// FIX 1: playWarningSound always resumes the AudioContext before playing,
// and uses a fresh oscillator chain every time so it never gets "stuck".
function playWarningSound(audioCtx) {
  ...
  const play = () => {
    try {
      ...
      // Use audioCtx.currentTime AFTER resume resolves so timing is fresh
  ...
  // Always force-resume regardless of assumed state, then play
...
export default function SafeCityPage() {
  ...
  // FIX 2: wakeLockRef MUST be inside the component, not at module level
  ...
  const initAudio = useCallback(() => {
    ...
    // Always try to resume — browsers suspend AudioContext after inactivity
    ...
    if (closest) {
      ...
      if (now - lastWarned > COOLDOWN_MS) {
        ...
        if (soundEnabledRef.current) {
          // FIX 1 continued: always resume before beeping
          if (audioCtxRef.current) {
            audioCtxRef.current.resume().then(() => {
              playWarningSound(audioCtxRef.current);
            }).catch(() => playWarningSound(audioCtxRef.current));
          }
        }
  ...
  // FIX 2: requestWakeLock and releaseWakeLock use the ref inside the component
  ...
  // Re-acquire wake lock when tab becomes visible again (browsers release it on tab switch)
  useEffect(() => {
  ...
  const startTracking = useCallback(() => {
    initAudio();
    if (!navigator.geolocation) { setTrackingError("Геолокацијата не е поддржана."); return; }
    setTrackingError(null); setTrackingActive(true);
    requestWakeLock(); // FIX 2: keep screen on when tracking starts
    watchIdRef.current = navigator.geolocation.watchPosition(
      ...
      (err) => {
        ...
        releaseWakeLock(); // FIX 2: release on error
      },
      { enableHighAccuracy: true, maximumAge: 2000, timeout: 10000 }
    );
  }, [initAudio, checkProximity, requestWakeLock, releaseWakeLock]);
  ...
  const stopTracking = useCallback(() => {
    ...
    releaseWakeLock(); // FIX 2: release when user stops tracking
  }, [releaseWakeLock]);
  ...
  const handleGpsToggle = () => {
    ...
    if (!trackingActive) {
      startTracking();
      setGpsHint(true);
      setTimeout(() => setGpsHint(false), 5000);
    } else {
      stopTracking();
      setGpsHint(false);
    }
  ...
  return (
    <>
      <Head>
        <title>Safe City — Makceni.mk</title>
        ...
        <header style={{ height: isMobile ? 48 : 60, flexShrink: 0, zIndex: 1000, background: "#FFFFFF", borderBottom: "1px solid #E8E5DE", display: "flex", alignItems: "center", justifyContent: "space-between", padding: isMobile ? "0 14px" : "0 24px", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: isMobile ? 8 : 12 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <span style={{ fontSize: isMobile ? 12 : 14, color: "#4A4640", fontWeight: 600 }}>Safe City</span>
            </div>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <div style={{ background: "#FEF2F2", border: "1px solid #FECACA", borderRadius: 8, padding: isMobile ? "4px 8px" : "5px 12px" }}>
              <span style={{ fontSize: isMobile ? 11 : 13, fontWeight: 700, color: "#DC2626" }}>{loading ? "..." : isMobile ? `${cameraCount}📷` : `${cameraCount} камери`}</span>
            </div>
            {!isMobile && (
              <>
                <button onClick={toggleReportMode} style={{ fontSize: 13, fontWeight: 700, color: reportMode ? "#fff" : "#F97316", background: reportMode ? "#F97316" : "#FFF7ED", border: `1px solid ${reportMode ? "#F97316" : "#FED7AA"}`, borderRadius: 8, padding: "6px 14px", cursor: "pointer", transition: "all 0.15s", fontFamily: "inherit" }}>
                  {reportMode ? "✕ Откажи" : "+ Пријави камера"}
                </button>
                <a href="/mapa" style={{ fontSize: 13, fontWeight: 600, color: "#92400E", background: "#FEF3C7", border: "1px solid #FDE68A", borderRadius: 8, padding: "6px 14px", textDecoration: "none" }}>Бензинcки</a>
              </>
            )}
            <a href="/" style={{ fontSize: isMobile ? 11 : 13, fontWeight: 500, color: "#9A9590", background: "#F5F4F1", border: "1px solid #E8E5DE", borderRadius: 8, padding: isMobile ? "4px 8px" : "6px 14px", textDecoration: "none" }}>←{!isMobile && " Назад"}</a>
          </div>
        </header>
        ...
        <div style={{ flex: 1, position: "relative" }}>
          ...
          {proximityAlert && (
            <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", zIndex: 700, background: "#FFFFFF", border: "2px solid #DC2626", borderRadius: 18, overflow: "hidden", width: "min(320px,88vw)", animation: "alertPulse 1.2s ease-out infinite,popIn 0.3s cubic-bezier(0.34,1.56,0.64,1) forwards" }}>
              <div style={{ background: "#DC2626", padding: "11px 16px", display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <span style={{ fontSize: 14, fontWeight: 800, color: "#fff", flex: 1 }}>Камера напред!</span>
                <button onClick={() => setProximityAlert(null)} style={{ background: "rgba(255,255,255,0.2)", border: "none", color: "#fff", borderRadius: 6, padding: "2px 8px", cursor: "pointer", fontSize: 14, fontWeight: 700, fontFamily: "inherit" }}>✕</button>
              </div>
              <div style={{ padding: "20px", textAlign: "center" }}>
                ...
                <div style={{ fontSize: 12, fontWeight: 700, color: "#9A9590", letterSpacing: 2, textTransform: "uppercase", marginBottom: 16 }}>метри</div>
                {proximityAlert.camera.maxspeed && (
                  <div style={{ display: "inline-flex", alignItems: "center", gap: 12, background: "#FEF2F2", border: "1px solid #FECACA", borderRadius: 12, padding: "10px 20px", marginBottom: 14 }}>
                    ...
                    <div style={{ textAlign: "left" }}>
                      <div style={{ fontSize: 10, fontWeight: 700, color: "#DC2626", letterSpacing: 1.5 }}>КМ/ЧАС</div>
                      <div style={{ fontSize: 10, color: "#9A9590", marginTop: 2 }}>ограничување</div>
                    </div>
                  </div>
                )}
                <div style={{ fontSize: 12, color: "#9A9590" }}>Намалете ја брзината</div>
              </div>
            </div>
          )}
          ...
          {trackingActive && (
            <div style={{ position: "absolute", bottom: 80, right: 16, zIndex: 600, background: "#FFFFFF", border: `2px solid ${getSpeedColor(currentSpeed)}`, borderRadius: 16, padding: "10px 16px", minWidth: 84, textAlign: "center", boxShadow: "0 4px 20px rgba(0,0,0,0.12)", transition: "border-color 0.3s" }}>
              ...
              <div style={{ fontSize: 10, fontWeight: 700, color: "#9A9590", letterSpacing: 1, textTransform: "uppercase", marginTop: 3 }}>км/ч</div>
            </div>
          )}
          ...
          {!isMobile && (
            <div style={{ position: "absolute", top: 16, left: 16, zIndex: 500, background: "rgba(255,255,255,0.97)", backdropFilter: "blur(12px)", border: "1px solid #E8E5DE", borderRadius: 14, padding: "16px 18px", maxWidth: 280, boxShadow: "0 4px 20px rgba(0,0,0,0.1)" }}>
              ...
              <div style={{ fontSize: 12, color: "#9A9590", lineHeight: 1.6, marginBottom: 10 }}>Локации на Safe City камери во Скопје.</div>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
                {[["#FF4444", "Камери"], ["#F97316", "Пријавени"], ["#3B82F6", "Ти"]].map(([color, label]) => (
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 14 }}>
                ...
                <span style={{ fontSize: 11, color: "#9A9590", fontWeight: 500 }}>{loading ? "Вчитување..." : `${cameraCount} локации`}</span>
              </div>
              <div style={{ borderTop: "1px solid #E8E5DE", paddingTop: 12, marginBottom: 12 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: "#9A9590", letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 8 }}>Предупредување</div>
                <button
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                    ...
                    <span>{trackingActive ? "GPS активен" : "Вклучи GPS"}</span>
                  </div>
                ...
                {/* FIX 3: Desktop GPS hint — shown when GPS is OFF */}
                {!trackingActive && !gpsHint && (
                  <div style={{ background: "#FFF7ED", border: "1px solid #FED7AA", borderRadius: 8, padding: "8px 10px", marginBottom: 6 }}>
                    <div style={{ fontSize: 11, fontWeight: 600, color: "#92400E" }}>💡 Вклучи GPS за да добиваш предупредувања за Safe City камери</div>
                  </div>
                )}
                ...
                <button onClick={() => { initAudio(); setSoundEnabled(s => !s); }} style={{ width: "100%", padding: "7px 12px", borderRadius: 9, cursor: "pointer", fontFamily: "inherit", fontSize: 12, fontWeight: 600, textAlign: "left", display: "flex", alignItems: "center", justifyContent: "space-between", background: "#F5F4F1", border: "1px solid #E8E5DE", color: "#4A4640", transition: "all 0.15s" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                    ...
                    <span style={{ color: "#9A9590" }}>Звук при приближување</span>
                  </div>
              ...
              <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 11, fontWeight: 600, color: "#9A9590", background: "#F5F4F1", border: "1px solid #E8E5DE", borderRadius: 6, padding: "4px 9px", textDecoration: "none" }}>© OpenStreetMap</a>
            </div>
          )}
          ...
          {isMobile && (
            <div style={{ position: "absolute", top: 12, left: "50%", transform: "translateX(-50%)", zIndex: 500, display: "flex", gap: 8 }}>
              <button onClick={handleGpsToggle} style={{ height: 40, padding: "0 14px", borderRadius: 20, background: trackingActive ? "#F0FDF4" : "#fff", border: `2px solid ${trackingActive ? "#86EFAC" : "#E8E5DE"}`, color: trackingActive ? "#15803D" : "#4A4640", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", boxShadow: "0 2px 12px rgba(0,0,0,0.12)", display: "flex", alignItems: "center", gap: 5, whiteSpace: "nowrap" }}>
                <span>📍</span>{trackingActive ? "GPS Вклучен" : "GPS"}
              </button>
          ...
          {/* FIX 3: Mobile GPS hint — shown when GPS is OFF */}
          {isMobile && !trackingActive && !gpsHint && !reportMode && (
            <div style={{ position: "absolute", top: 62, left: "50%", transform: "translateX(-50%)", zIndex: 500, background: "#fff", border: "1px solid #FED7AA", borderRadius: 10, padding: "8px 14px", fontSize: 11, color: "#92400E", fontWeight: 600, whiteSpace: "nowrap", boxShadow: "0 2px 10px rgba(0,0,0,0.08)" }}>
              💡 Вклучи GPS за предупредувања за камери
            </div>
          )}
          ...
          {reportPin && !reportSent && (
            <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, zIndex: 500, background: "#FFFFFF", borderRadius: isMobile ? "20px 20px 0 0" : 18, padding: isMobile ? "20px 16px 36px" : "20px 22px", boxShadow: "0 -4px 24px rgba(0,0,0,0.1)", animation: isMobile ? "slideUp 0.25s ease" : "fadeUp 0.2s ease" }}>
              ...
              <div style={{ fontSize: 16, fontWeight: 800, color: "#1A1814", marginBottom: 4 }}>Пријави камера</div>
              <div style={{ fontSize: 12, color: "#9A9590", marginBottom: 16 }}>Локација: {reportPin.lat.toFixed(5)}, {reportPin.lng.toFixed(5)}</div>
              <div style={{ marginBottom: 12 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: "#9A9590", letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 5 }}>Ограничување на брзина</div>
              ...
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: "#9A9590", letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 5 }}>Опис (опционално)</div>
                <input type="text" placeholder="пр. Бул. Партизански Одреди" value={reportForm.description} onChange={e => setReportForm(f => ({ ...f, description: e.target.value }))} style={{ width: "100%", padding: "9px 12px", borderRadius: 9, border: "1px solid #E8E5DE", background: "#F5F4F1", fontSize: 13, color: "#1A1814", outline: "none" }} />
              </div>
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => { setReportPin(null); if (reportMarkerRef.current) { reportMarkerRef.current.remove(); reportMarkerRef.current = null; } }} style={{ flex: 1, padding: "10px", borderRadius: 9, border: "1px solid #E8E5DE", background: "#F5F4F1", color: "#9A9590", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>Откажи</button>
                <button onClick={submitReport} disabled={reportLoading} style={{ flex: 2, padding: "10px", borderRadius: 9, border: "none", background: "#F97316", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", opacity: reportLoading ? 0.7 : 1, fontFamily: "inherit" }}>{reportLoading ? "Праќање..." : "Пријави камера →"}</button>
              </div>
            </div>
          )}
            ...
            <div style={{ position: "absolute", bottom: isMobile ? 0 : 24, left: isMobile ? 0 : "50%", right: isMobile ? 0 : "auto", transform: isMobile ? "none" : "translateX(-50%)", zIndex: 500, background: "#FFFFFF", borderRadius: isMobile ? "20px 20px 0 0" : 18, padding: isMobile ? "16px 16px 36px" : "20px 22px", boxShadow: "0 -4px 24px rgba(0,0,0,0.1)", animation: isMobile ? "slideUp 0.25s ease" : "fadeUp 0.2s ease", minWidth: isMobile ? "auto" : 300, maxWidth: isMobile ? "100%" : 400 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 15, fontWeight: 800, color: "#1A1814" }}>Камера за брзина</div>
                    <div style={{ fontSize: 11, color: "#9A9590", marginTop: 2 }}>{selectedCamera.reported ? "Пријавена од корисник" : "Safe City · OSM"}</div>
                  </div>
                </div>
              ...
              {selectedCamera.maxspeed ? (
                <div style={{ background: "#FEF2F2", border: "1px solid #FECACA", borderRadius: 12, padding: "14px 16px", marginBottom: 14, display: "flex", alignItems: "center", gap: 16 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 700, color: "#DC2626", letterSpacing: 1 }}>КМ/ЧАС</div>
                    <div style={{ fontSize: 12, color: "#9A9590", marginTop: 2 }}>Максимална брзина</div>
                  </div>
                </div>
              ) : (
                <div style={{ background: "#FEF2F2", border: "1px solid #FECACA", borderRadius: 12, padding: "11px 14px", marginBottom: 14 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: "#DC2626" }}>⚠️ Возете внимателно</div>
                </div>
              )}
              <a href={`https://www.google.com/maps/dir/?api=1&destination=${selectedCamera.lat},${selectedCamera.lng}`} target="_blank" rel="noopener noreferrer" style={{ display: "block", textAlign: "center", background: "#F5F4F1", border: "1px solid #E8E5DE", borderRadius: 10, padding: "10px", color: "#4A4640", fontSize: 13, fontWeight: 600, textDecoration: "none" }}>Отвори во Google Maps →</a>
            </div>
          )}
        </div>
      </div>
