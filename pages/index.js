...
function ShareButton({ fuel }) {
  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      ...
      >
        <span style={{ fontSize: 14 }}>📤</span> Сподели
      </button>
      {showMenu && (
        <div style={{ position: "absolute", top: "calc(100% + 8px)", right: 0, background: "#fff", border: `1px solid ${C.border}`, borderRadius: 12, overflow: "hidden", boxShadow: "0 8px 32px rgba(0,0,0,0.12)", zIndex: 100, minWidth: 160, animation: "fadeUp 0.15s ease" }}>
          ...
          <button onClick={() => { navigator.clipboard?.writeText(message); setShowMenu(false); }}
            ...
            onMouseEnter={e => e.currentTarget.style.background = C.surface2}
            onMouseLeave={e => e.currentTarget.style.background = "transparent"}
          ><span style={{ fontSize: 18 }}>📋</span> Копирај</button>
        </div>
      )}
    </div>
  );
...
function CarouselCard({ fuel, position, onClick, timeStr, loading }) {
    ...
    <div onClick={() => !isCenter && onClick()} style={{ position: "absolute", left: "50%", top: "50%", width: 340, transform: `translateX(calc(-50% + ${position * 310}px)) translateY(-50%) scale(${isCenter ? 1 : 0.78})`, transition: "all 0.5s cubic-bezier(0.34, 1.56, 0.64, 1)", opacity: isCenter ? 1 : 0.6, filter: isAdjacent ? "blur(1.5px)" : "none", zIndex: isCenter ? 10 : 5, cursor: isCenter ? "default" : "pointer" }}>
      <div style={{ background: fc.bg, borderRadius: 24, padding: "28px 28px 20px", boxShadow: isCenter ? `0 28px 70px ${fc.bg}55, 0 0 0 1px rgba(255,255,255,0.1)` : "0 12px 32px rgba(0,0,0,0.15)", minHeight: 280, display: "flex", flexDirection: "column", justifyContent: "space-between", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
          <div>
            <div style={{ fontSize: 22, fontWeight: 800, color: "#fff", letterSpacing: -0.3 }}>{fuel.label}</div>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            {isCenter && <ShareButton fuel={fuel} />}
        ...
        <div style={{ marginBottom: 8 }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
            ...
            <span style={{ fontSize: 18, fontWeight: 600, color: "rgba(255,255,255,0.5)" }}>ден</span>
          </div>
          <div style={{ fontSize: 12, color: "rgba(255,255,255,0.35)", marginTop: 3 }}>{fuel.unit}</div>
          {hasChange ? <div style={{ fontSize: 20, fontWeight: 800, color: fc.accent, marginTop: 8 }}>{fuel.change > 0 ? "▲ +" : "▼ "}{fuel.change.toFixed(1)} ден</div>
            : <div style={{ fontSize: 13, color: "rgba(255,255,255,0.3)", marginTop: 8 }}>Нема промена</div>}
        </div>
...
function MobileCarousel({ fuelData, activeIdx, onSelect, timeStr, loading }) {
              ...
              <div key={`${fuel.key}-${i}`} style={{ flex: `0 0 ${cardW}`, transition: dragging ? "none" : "opacity 0.3s, transform 0.3s", opacity: isActive ? 1 : 0.45, transform: isActive ? "scale(1)" : "scale(0.95)" }}>
                <div style={{ background: fc.bg, borderRadius: 20, padding: "22px 22px 16px", boxShadow: isActive ? `0 18px 48px ${fc.bg}55, 0 0 0 1px rgba(255,255,255,0.1)` : "0 6px 18px rgba(0,0,0,0.1)", minHeight: 250, display: "flex", flexDirection: "column", justifyContent: "space-between", position: "relative", overflow: "hidden", userSelect: "none", WebkitUserSelect: "none" }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                    <div>
                      <div style={{ fontSize: 20, fontWeight: 800, color: "#fff", letterSpacing: -0.3 }}>{fuel.label}</div>
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4 }}>
                      {isActive && <ShareButton fuel={fuel} />}
                  ...
                  <div style={{ marginBottom: 6 }}>
                    <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
                      ...
                      <span style={{ fontSize: 16, fontWeight: 600, color: "rgba(255,255,255,0.5)" }}>ден</span>
                    </div>
                    <div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", marginTop: 2 }}>{fuel.unit}</div>
                    {hasChange ? <div style={{ fontSize: 17, fontWeight: 800, color: fc.accent, marginTop: 6 }}>{fuel.change > 0 ? "▲ +" : "▼ "}{fuel.change.toFixed(1)} ден</div>
                      : <div style={{ fontSize: 12, color: "rgba(255,255,255,0.3)", marginTop: 6 }}>Нема промена</div>}
                  </div>
...
// ── Price History Chart ────────────────────────────────
function PriceHistory({ fuelData, isMobile }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/prices?history=1&period=${periodMap[activePeriod]}`)
      ...
      .then(d => {
        ...
        setHistoryData(entries.length >= 2 ? entries : null);
    ...
    const renderChart = () => {
      ...
      const labels = isRealData && historyData
        ? historyData.map(entry => {
            ...
            if (activePeriod === "7д") return d.toLocaleDateString("mk-MK", { weekday: "short" });
            if (activePeriod === "30д") return d.toLocaleDateString("mk-MK", { day: "numeric", month: "short" });
            return d.toLocaleDateString("mk-MK", { month: "short" });
          })
        : data.map((_, i) => {
            if (activePeriod === "7д") { const d = new Date(); d.setDate(d.getDate() - (data.length - 1 - i)); return d.toLocaleDateString("mk-MK", { weekday: "short" }); }
      ...
      chartRef.current = new window.Chart(ctx, {
        ...
        options: {
          ...
          plugins: {
            ...
            tooltip: { backgroundColor: "#1C1917", titleColor: "#A8A29E", bodyColor: "#fff", bodyFont: { size: 14, weight: "700" }, padding: 10, cornerRadius: 8, callbacks: { label: (ctx) => ` ${ctx.raw.toFixed(1)} ден` } },
          },
          scales: {
  ...
  return (
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, overflow: "hidden" }}>
      <div style={{ padding: "16px 20px", background: C.surface2, borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
        <div>
          <div style={{ fontSize: 15, fontWeight: 700, color: C.text }}>Историја на цени</div>
          <div style={{ fontSize: 12, color: C.muted, marginTop: 1 }}>{isRealData ? "Реални податоци од gorivo.mk" : "Движење на цените низ времето"}</div>
        </div>
      ...
      <div style={{ padding: "14px 20px", borderBottom: `1px solid ${C.border}`, display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8 }}>
        {[
          { label: "Тековна", value: `${currentPrice} ден`, color: C.text },
          { label: "Минимум", value: `${minPrice} ден`, sub: activePeriod, color: C.green },
          { label: "Максимум", value: `${maxPrice} ден`, sub: activePeriod, color: C.red },
          { label: "Промена", value: `${diffPositive ? "+" : ""}${diff} ден`, color: diffPositive ? C.red : diffNegative ? C.green : C.text },
      ...
      {!isRealData && (
        <div style={{ padding: "8px 20px 12px", fontSize: 11, color: C.muted, borderTop: `1px solid ${C.border}` }}>
          ⓘ Историски податоци се собираат од денес. Реалниот график ќе биде достапен по неколку дена.
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, overflow: "hidden" }}>
      <div style={{ padding: "16px 20px", background: C.surface2, borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <div style={{ fontSize: 15, fontWeight: 700, color: C.text }}>🚗 Мојот автомобил</div>
          <div style={{ fontSize: 12, color: C.muted, marginTop: 1 }}>Персонализирани трошоци за возење</div>
        </div>
        {profile && !editing && (
          <div style={{ display: "flex", gap: 6 }}>
            <button onClick={() => setEditing(true)} style={{ padding: "5px 12px", borderRadius: 7, border: `1px solid ${C.border}`, background: C.surface, color: C.textMid, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>Промени</button>
            <button onClick={clear} style={{ padding: "5px 12px", borderRadius: 7, border: `1px solid ${C.redBdr}`, background: C.redBg, color: C.red, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>Избриши</button>
          </div>
        )}
      </div>
        ...
        {!profile || editing ? (
          <div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
              <div>
                <div style={{ fontSize: 10, fontWeight: 700, color: C.muted, letterSpacing: 0.8, textTransform: "uppercase", marginBottom: 5 }}>Модел на возило</div>
                <input type="text" placeholder="пр. VW Golf 5" value={form.model} onChange={e => setForm(f => ({ ...f, model: e.target.value }))} style={inp} />
              </div>
              <div>
                <div style={{ fontSize: 10, fontWeight: 700, color: C.muted, letterSpacing: 0.8, textTransform: "uppercase", marginBottom: 5 }}>Потрошувачка (л/100км)</div>
            ...
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: C.muted, letterSpacing: 0.8, textTransform: "uppercase", marginBottom: 5 }}>Вид гориво</div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {editing && <button onClick={() => setEditing(false)} style={{ flex: 1, padding: "10px", borderRadius: 9, border: `1px solid ${C.border}`, background: C.surface2, color: C.textMid, fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>Откажи</button>}
              <button onClick={save} style={{ flex: 2, padding: "10px", borderRadius: 9, border: "none", background: C.orange, color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>{editing ? "Зачувај промени" : "Зачувај профил →"}</button>
            </div>
          </div>
        ) : (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
              <div style={{ background: C.orangeBg, border: `1px solid ${C.orangeBdr}`, borderRadius: 12, padding: "14px 16px" }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: C.orange, letterSpacing: 0.8, textTransform: "uppercase", marginBottom: 4 }}>Трошок / км</div>
              ...
              <div style={{ background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 12, padding: "14px 16px" }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: C.muted, letterSpacing: 0.8, textTransform: "uppercase", marginBottom: 4 }}>Трошок / 100км</div>
...
function MobileDrawer({ open, close, loading, timeStr }) {
  return (
    <>
      ...
      <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, width: "80%", maxWidth: 300, background: C.surface, zIndex: 999, boxShadow: "-12px 0 48px rgba(0,0,0,0.18)", transform: open ? "translateX(0)" : "translateX(100%)", transition: "transform 0.32s cubic-bezier(0.16,1,0.3,1)", display: "flex", flexDirection: "column", overflowY: "auto" }}>
        ...
        <div style={{ padding: "10px 20px", background: C.surface2, borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 7 }}>
          ...
          <span style={{ fontSize: 12, color: C.muted, fontWeight: 500 }}>{loading ? "Вчитување..." : timeStr ? `Ажурирано ${timeStr}` : "Во живо"}</span>
        </div>
        <nav style={{ padding: "8px 14px" }}>
          {[["Цени", "#ceni"], ["Калкулатор", "#calculator"], ["Историја", "#history"], ["Берза", "#berza"], ["Вести", "#news"]].map(([label, href]) => (
        ...
        <div style={{ padding: "14px 14px", display: "flex", flexDirection: "column", gap: 10, marginTop: 4 }}>
          <a href="/mapa" onClick={close} style={{ display: "flex", alignItems: "center", gap: 12, padding: "15px 18px", borderRadius: 13, background: "#FEF3C7", border: "1px solid #FDE68A", textDecoration: "none", fontWeight: 700, fontSize: 16, color: "#92400E" }}><span style={{ fontSize: 22 }}>⛽</span>Бензинcки</a>
          <a href="/safecity" onClick={close} style={{ display: "flex", alignItems: "center", gap: 12, padding: "15px 18px", borderRadius: 13, background: "#FEF2F2", border: "1px solid #FECACA", textDecoration: "none", fontWeight: 700, fontSize: 16, color: "#991B1B" }}><span style={{ fontSize: 22 }}>📷</span>Safe City</a>
        </div>
      </div>
    </>
  );
...
function Berza() {
  ...
  const oil = data?.oil || [
    { name: "Brent Crude", usd: "—", mkd: "—", change: 0, unit: "барел" },
    { name: "WTI Crude",   usd: "—", mkd: "—", change: 0, unit: "барел" },
  ];
  const metals = data?.metals || [
    { name: "Злато",  usd: "—", mkd: "—", change: 0, unit: "грам" },
    { name: "Сребро", usd: "—", mkd: "—", change: 0, unit: "грам" },
  ];
    ...
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderBottom: last ? "none" : `1px solid ${C.border}` }}>
      <div>
        ...
        <div style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>
          {loading ? "Вчитување..." : `${typeof item.mkd === "number" ? item.mkd.toLocaleString() : item.mkd} ден / ${item.unit}`}
        </div>
      </div>
      <div style={{ textAlign: "right" }}>
        ...
        {!loading && typeof item.change === "number" && item.change !== 0 && (
          <div style={{ fontSize: 11, fontWeight: 700, color: item.change > 0 ? C.red : C.green, marginTop: 2 }}>
            {item.change > 0 ? "▲ +" : "▼ "}{Math.abs(item.change)}% денес
          </div>
        )}
      </div>
    </div>
  ...
  const Section = ({ emoji, title, items, border }) => (
    <div style={{ padding: "0 20px", borderBottom: border ? `1px solid ${C.border}` : "none" }}>
      <div style={{ fontSize: 10, fontWeight: 700, color: C.muted, letterSpacing: 1, textTransform: "uppercase", paddingTop: 12, paddingBottom: 2 }}>
        {emoji} {title}
      </div>
  ...
  return (
    <div style={{ background: `linear-gradient(135deg, ${C.orangeBg}, #FFF0E0)`, border: `1px solid ${C.orangeBdr}`, borderRadius: 16, padding: "22px 22px" }}>
      <div style={{ marginBottom: 14 }}>
        <div style={{ fontSize: 16, fontWeight: 700, color: C.text, marginBottom: 4 }}>Известувања за цени</div>
        <div style={{ fontSize: 13, color: C.textMid, lineHeight: 1.6 }}>Прими известување кога цената ќе падне под твојот праг.</div>
      </div>
      {sent ? <div style={{ fontSize: 15, fontWeight: 600, color: C.green }}>✓ Зачувано!</div> : (
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <input type="email" placeholder="email@example.com" value={email} onChange={e => setEmail(e.target.value)} style={{ flex: "1 1 160px", padding: "10px 14px", borderRadius: 9, border: `1px solid ${C.orangeBdr}`, background: "#fff", fontSize: 14, outline: "none" }} />
          <button onClick={() => setSent(true)} style={{ padding: "10px 20px", borderRadius: 9, background: C.orange, color: "#fff", fontSize: 14, fontWeight: 700, border: "none", cursor: "pointer", whiteSpace: "nowrap" }}>Активирај</button>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, overflow: "hidden" }}>
      <div style={{ padding: "16px 20px", background: C.surface2, borderBottom: `1px solid ${C.border}` }}>
        <div style={{ fontSize: 15, fontWeight: 700, color: C.text }}>Калкулатор за пат</div>
        <div style={{ fontSize: 12, color: C.muted, marginTop: 1 }}>Пресметај ги трошоците за патување</div>
      </div>
      <div style={{ padding: "18px 20px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
          <div>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <select value={fuelKey} onChange={e => setFuelKey(e.target.value)} style={{ ...inp, cursor: "pointer" }}>
            {fuelData.map(f => <option key={f.key} value={f.key}>{f.label} — {f.price} {f.unit}</option>)}
          </select>
        </div>
        <div style={{ background: C.orangeBg, border: `1px solid ${C.orangeBdr}`, borderRadius: 12, padding: "14px 18px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
          <div>
            <div style={{ fontSize: 10, fontWeight: 700, color: C.orange, letterSpacing: 0.8, textTransform: "uppercase", marginBottom: 4 }}>Вкупен трошок</div>
            <div style={{ fontSize: 42, fontWeight: 800, color: C.orange, letterSpacing: -2, lineHeight: 1 }}>
              {cost}<span style={{ fontSize: 14, fontWeight: 400, color: C.muted, marginLeft: 4 }}>ден</span>
            </div>
          </div>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: C.muted, letterSpacing: 0.8, textTransform: "uppercase", marginBottom: 4 }}>Литри</div>
            <div style={{ fontSize: 26, fontWeight: 800, color: C.text, letterSpacing: -1 }}>
              {liters}<span style={{ fontSize: 12, fontWeight: 400, color: C.muted, marginLeft: 2 }}>л</span>
            </div>
          </div>
        </div>
      </div>
...
export default function Home() {
  ...
  const [fuelData, setFuelData] = useState(FALLBACK);
  const [loading, setLoading] = useState(true);
  const [updatedAt, setUpdatedAt] = useState(null);
  ...
  useEffect(() => {
    fetch("/api/news")
      .then(r => r.json())
      .then(d => { const fetched = d.news || []; setNews(fetched); setOriginalNews(fetched); setNewsLoading(false); })
  ...
  const today = new Date().toLocaleDateString("mk-MK", { day: "numeric", month: "long", year: "numeric" });
  ...
  return (
    <>
      ...
      <div style={{ minHeight: "100vh", background: C.bg }}>
          ...
          <div style={{ maxWidth: 1180, margin: "0 auto", padding: `0 ${px}`, height: 60, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            {!isMobile && (
              <nav style={{ display: "flex", alignItems: "center", gap: 4 }}>
                ...
                <a href="/mapa" style={{ padding: "7px 13px", borderRadius: 9, fontSize: isTablet ? 13 : 15, fontWeight: 700, color: "#92400E", background: "#FEF3C7", border: "1px solid #FDE68A", textDecoration: "none" }}>Бензинcки</a>
                <a href="/safecity" style={{ padding: "7px 13px", borderRadius: 9, fontSize: isTablet ? 13 : 15, fontWeight: 700, color: "#991B1B", background: "#FEF2F2", border: "1px solid #FECACA", textDecoration: "none" }}>Safe City</a>
              </nav>
            )}
            {!isMobile && (
              <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                {loading ? <div style={{ width: 7, height: 7, borderRadius: "50%", border: `2px solid ${C.orange}`, borderTopColor: "transparent", animation: "spin 0.7s linear infinite" }} /> : <LiveDot />}
                <span style={{ fontSize: 13, color: C.muted, fontWeight: 500 }}>{loading ? "Вчитување..." : timeStr ? `Аж. ${timeStr}` : "Во живо"}</span>
              </div>
            )}
            {isMobile && (
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        {isMobile && <MobileDrawer open={drawer.open} close={drawer.close} loading={loading} timeStr={timeStr} />}
        ...
        <div id="ceni" style={{ position: "relative", zIndex: 1, maxWidth: 1180, margin: "0 auto", padding: `${isMobile ? "80px" : "100px"} ${px} 0` }}>
          ...
          {isMobile ? (
            <div style={{ marginBottom: 32 }}>
              <MobileCarousel fuelData={fuelData} activeIdx={activeIdx} onSelect={handleSelect} timeStr={timeStr} loading={loading} />
            </div>
          ) : (
            <>
              <div style={{ position: "relative", height: 360, marginBottom: 36 }}>
                {fuelData.map((fuel, i) => {
                  ...
                  return <CarouselCard key={fuel.key} fuel={fuel} position={wrapped} timeStr={timeStr} loading={loading} onClick={() => handleSelect(i)} />;
                })}
              </div>
        ...
        <main style={{ position: "relative", zIndex: 1, maxWidth: 1180, margin: "0 auto", padding: `36px ${px} 80px` }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 14, marginBottom: 40 }}>
            ...
            >
              <div style={{ height: 130, background: "linear-gradient(135deg, #FEF3C7 0%, #FDE68A 100%)", position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ position: "absolute", top: 14, right: 14, background: "#92400E", color: "#FEF3C7", fontSize: 11, fontWeight: 700, borderRadius: 6, padding: "3px 9px" }}>350+ локации</div>
              </div>
              <div style={{ padding: "14px 18px" }}>
                ...
                <div style={{ fontSize: 13, fontWeight: 700, color: "#D97706" }}>Отвори мапа →</div>
              </div>
            </a>
            ...
            >
              <div style={{ height: 130, background: "linear-gradient(135deg, #FEF2F2 0%, #FECACA 100%)", position: "relative", overflow: "hidden" }}>
                <div style={{ position: "absolute", bottom: 14, left: 16, display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <span style={{ fontSize: 20, fontWeight: 800, color: "#991B1B" }}>Safe City</span>
                </div>
                <div style={{ position: "absolute", top: 14, right: 14, background: "#991B1B", color: "#FEF2F2", fontSize: 11, fontWeight: 700, borderRadius: 6, padding: "3px 9px" }}>218 камери</div>
              </div>
              <div style={{ padding: "14px 18px" }}>
                <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.6, marginBottom: 8 }}>Локации на Safe City камери за брзина во Скопје. Возете внимателно.</div>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#DC2626" }}>Отвори мапа →</div>
              </div>
            </a>
          </div>

          <div style={{ height: 1, background: C.border, marginBottom: 36 }} />
          <div style={{ marginBottom: 36 }}><AlertBanner /></div>
          <div style={{ height: 1, background: C.border, marginBottom: 36 }} />
          <div id="calculator" style={{ marginBottom: 36 }}><Calculator fuelData={fuelData} /></div>
          <div style={{ height: 1, background: C.border, marginBottom: 36 }} />
          <div style={{ marginBottom: 36 }}><CarProfile fuelData={fuelData} /></div>
          <div style={{ height: 1, background: C.border, marginBottom: 36 }} />
          <div id="history" style={{ marginBottom: 36 }}><PriceHistory fuelData={fuelData} isMobile={isMobile} /></div>
          <div style={{ height: 1, background: C.border, marginBottom: 36 }} />
          <div id="berza" style={{ marginBottom: 36 }}><Berza /></div>
          <div style={{ height: 1, background: C.border, marginBottom: 36 }} />

          <div style={{ marginBottom: 14 }}>
            <span id="news" style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", color: C.muted }}>Вести</span>
          </div>
          ...
          {newsLoading ? (
            ...
            <div style={{ textAlign: "center", padding: "40px 20px", color: C.muted, fontSize: 14 }}>Нема вести во моментов.</div>
          ) : (
            <>
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(auto-fill, minmax(260px, 1fr))", gap: 12 }}>
              ...
              <div style={{ display: "flex", justifyContent: "center", marginTop: 20 }}>
                ...
                >{showAllNews ? "← Прикажи помалку" : "Прикажи повеќе вести →"}</button>
              </div>
            </>
          )}
        </main>
